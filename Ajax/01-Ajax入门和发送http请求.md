...
（2）与服务器建立连接: 使用XMLHttpRequest 对象的open方法设置请求的参数。open(method, url, async)。参数解释：请求的方法、请求的url、是否异步。

（3）发送请求: 通过 XMLHttpRequest 对象的 send() 方法，将客户端页面的数据发送给服务端 xhr.send([body])

（4）注册事件。 注册onreadystatechange事件，状态改变时就会调用，主要监听的属性为XMLHttpRequest.readyState。
